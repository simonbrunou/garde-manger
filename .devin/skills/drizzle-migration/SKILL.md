...
description: Create and apply a Drizzle schema migration for garde-manger (SQLite). Use when adding or changing a table or column.
allowed-tools:
  - read
  - grep
  - glob
...
This writes a new numbered `.sql` file in `drizzle/` and updates `drizzle/meta`. If you made no schema change it prints "No schema changes, nothing to migrate".
